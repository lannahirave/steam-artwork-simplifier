...
describe('split batch quality recovery', () => {
  it('recovers under-budget split parts to higher quality values', async () => {
    ...
    const result = await recoverSplitBatchQuality({
      ...
      runFixedSplitPartQualityProbe: async (partIndex, fps, quality) => {
        attempts.push({ partIndex, fps, quality })
        const base = [4181.8, 2537.7, 3354.5, 2738.0, 4160.3][partIndex]
        const sizeGrowthKbByQuality = [48, 36, 32, 40, 24][partIndex]
        return artifact(partIndex + 1, base + (quality - 68) * sizeGrowthKbByQuality, quality, fps)
      },
    })
  ...
  it('keeps best accepted intermediate quality when the coarse ladder overshoots', async () => {
    ...
    const result = await recoverSplitBatchQuality({
      ...
      runFixedSplitPartQualityProbe: async (partIndex, fps, quality) => {
        ...
        return artifact(partIndex + 1, quality === 76 ? 4700 : 3354.5 + (quality - 68) * 140, quality, fps)
      },
    })
  ...
  it('logs quality recovery details', async () => {
    ...
    await recoverSplitBatchQuality({
      ...
      runFixedSplitPartQualityProbe: async (partIndex, fps, quality) => artifact(partIndex + 1, 4200, quality, fps),
    })
  ...
  it('recovers shared fps only when all parts still fit', async () => {
    const result = await recoverSplitBatchQuality({
      ...
      runFixedSplitPartQualityProbe: async (partIndex, fps, quality) => artifact(partIndex + 1, 4300, quality, fps),
    })
  ...
  it('runs independent per-part quality recovery in parallel', async () => {
    ...
    const result = await recoverSplitBatchQuality({
      ...
      runFixedSplitPartQualityProbe: async (partIndex, fps, quality) => {
        ...
        activeSearches -= 1
        return artifact(partIndex + 1, 4300, quality, fps)
      },
    })
  ...
  it('probes every active part in a wave before advancing a part search', async () => {
    const attempts: Array<{ partIndex: number; quality: number }> = []

    await recoverSplitBatchQuality({
      items: [artifact(1, 3000, 68), artifact(2, 3200, 68)],
      partOrder: [0, 1],
      label: 'Workshop',
      targetGifKb: 4500,
      maxGifKb: 5000,
      ...
      runFixedSplitPart: async (partIndex, fps, quality) => artifact(partIndex + 1, 4300, quality, fps),
      runFixedSplitPartQualityProbe: async (partIndex, fps, quality) => {
        attempts.push({ partIndex, quality })
        await new Promise((resolve) => setTimeout(resolve, 1))
        return artifact(partIndex + 1, quality <= 84 ? 4300 : 4700, quality, fps)
      },
    })
