(() => {
  ...
  function applyFeaturedFields() {
    if (typeof window.$J === "function") {
      $J("[name=image_width]").val(1000);
      $J("[name=image_height]").val(1);
      $J("[name=visibility]").val(0);
      return;
    }

    setInputValue("[name='image_width']", 1000);
    setInputValue("[name='image_height']", 1);
  ...
  console.log("[steam_upload_autofill featured] done", {
    titleSet: !!titleInput,
    agreeSet,
    fields: {
      image_width: 1000,
